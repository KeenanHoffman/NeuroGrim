...
- **Principle alignment.** Feature-gating conformance suites is **VISION #20 in action** ("Pluggability by use, not aspiration" — landed at V5-DOC-2 2026-05-04 via dual-review T+P; revised from initial draft "Pluggability is justified by use, not aspiration") — the four in-tree example crates + two workspace crates (`neurogrim-sensory`, `neurogrim-ecosystem`) ARE the real use that justifies the conformance pluggability. Pre-1.0, this earns its place. Also advances **VISION #8** (absorption over invention — extending the optional-dep pattern, not inventing one).
