...
pub struct CheckCodeQualityParams {
...
impl CodeQualityServer {
    #[tool(
        description = "Check code quality: README, lint configs, editor config. Returns CMDB-envelope JSON."
    )]
    async fn check_code_quality(
        &self,
        Parameters(p): Parameters<CheckCodeQualityParams>,
    ) -> String {
        serde_json::to_string_pretty(&analyze_code_quality(&p.project_root).await)
            .unwrap_or_default()
    }
}
...
impl ServerHandler for CodeQualityServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            instructions: Some("Code quality sensory tool.".into()),
...
pub async fn analyze_code_quality(project_root: &str) -> Value {
    ...
    if lc > 0 {
        score += 25;
        findings.push(Finding {
            name: "lint_config".into(),
            status: "found".into(),
            points: 25,
            detail: Some(format!("{} config(s)", lc)),
        });
    }
