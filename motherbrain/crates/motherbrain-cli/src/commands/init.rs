//! `motherbrain init` — project scanner and registry generator.
//!
...
pub fn generate_registry(
    ...
    json!({
        "meta": {
            "schema_version": "2.0",
            "description": format!("Brain registry for {} — generated by motherbrain init", project_name),
        ...
        "config": {
            ...
            "hats": {},
            "sensory_servers": {
                "_doc": "Add external MCP sensory servers here. Example: { \"command\": \"python\", \"args\": [\"-m\", \"my_tool\"], \"transport\": \"stdio\" }. See https://github.com/keenanHoffmanSparq/LSP-Brains"
            }
        }
    })
}
...
pub async fn run(project_root: &str, output: &str, yes: bool) -> Result<()> {
    ...
    if output_path.exists() {
        bail!(
            "Registry already exists at '{}'. Use 'motherbrain validate' to check it, \
    ...
    if !awareness_path.exists() {
        let empty = motherbrain_core::awareness::LocalAwareness::empty();
    ...
    println!();
    println!("Next steps:");
    println!("  motherbrain validate     # verify configuration");
    println!("  motherbrain sensory test-health && motherbrain sensory code-quality && motherbrain sensory deploy-readiness");
    println!("  motherbrain score        # get your first score");
    println!("  motherbrain health       # full dashboard");
    println!();
    println!("Local awareness:");
    println!("  motherbrain awareness    # view machine-specific facts agents have recorded");
    println!("  motherbrain awareness add --key tool_paths.cargo --value /path/to/cargo --category tool_paths");
