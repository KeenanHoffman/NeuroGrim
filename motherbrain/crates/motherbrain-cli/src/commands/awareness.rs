//! `motherbrain awareness` — manage local machine-specific facts and notes.
//!
...
//! ```bash
//! # Add a fact
//! motherbrain awareness add --key tool_paths.cargo \
//!   --value "C:\Users\...\.rustup\toolchains\stable\bin\cargo.exe" \
//!   --category tool_paths \
//!   --note "Not on PATH in bash; invoke via full path"
//!
//! # Add a free-form note
//! motherbrain awareness note "cargo test exits 1 on Windows due to file lock warning — tests actually pass" \
//!   --category patterns
//!
//! # List everything
//! motherbrain awareness
//!
//! # Get a specific fact
//! motherbrain awareness get tool_paths.cargo
//! ```

use anyhow::{Context, Result};
use clap::Subcommand;
use colored::Colorize;
use motherbrain_core::awareness::{AwarenessCategory, LocalAwareness};
use std::path::Path;
use std::str::FromStr;

/// Sub-commands for `motherbrain awareness`.
...
async fn cmd_list(root: &Path) -> Result<()> {
    ...
    if a.is_empty() {
        ...
        println!();
        println!("Add facts:  motherbrain awareness add --key KEY --value VALUE");
        println!("Add notes:  motherbrain awareness note \"text\"");
        return Ok(());
    }
