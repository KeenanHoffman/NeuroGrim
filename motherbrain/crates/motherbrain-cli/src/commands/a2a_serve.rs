//! `motherbrain a2a-serve` — serve this Brain as an A2A peer (spec §13, G.2, G.4).
//!
...
use motherbrain_a2a::envelope::{A2aEnvelope, MessageType};
use motherbrain_a2a::error::A2aError;
use motherbrain_a2a::{AgentCard, TaskServer};
...
pub fn build_agent_card_from_registry(
    ...
    let id = if registry.meta.updated_by.trim().is_empty() {
        ...
        format!("motherbrain-{}", uuid::Uuid::new_v4())
    } else {
...
pub async fn run(
    ...
    tracing::info!(
        ...
        endpoint = %endpoint,
        bind = %addr,
        "Moth(er):Br+AI+n A2A server starting — peers can discover this Brain"
    );
    eprintln!("Moth(er):Br+AI+n A2A server");
...
mod tests {
    use super::*;
    use motherbrain_core::registry::{BrainConfig, RegistryMeta};
    ...
    fn agent_card_falls_back_to_uuid_when_meta_is_blank() {
        ...
        let card = build_agent_card_from_registry(&reg, "http://127.0.0.1:8421/a2a/v1/", "0.1.0");
        assert!(
            card.id.starts_with("motherbrain-"),
