...
/// `motherbrain_ecosystem::invoke_child` so there's only one A2A-client
...
async fn load_a2a_domain(
    domain_key: &str,
    src: &motherbrain_core::registry::ScoringSource,
) -> Option<CmdbData> {
    use motherbrain_core::ecosystem::{ChildEntry, ChildTransport};
    use motherbrain_ecosystem::invoke_child;
