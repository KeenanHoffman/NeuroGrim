//! `motherbrain a2a-discover` — fetch and pretty-print a peer Brain's Agent
//! Card.
//!
...
use motherbrain_a2a::TaskClient;
use url::Url;
...
pub async fn run(peer_url: String) -> Result<()> {
    ...
    println!(
        ...
        match card.transport.protocol {
            motherbrain_a2a::agent_card::TransportProtocol::HttpSse => "http+sse",
            motherbrain_a2a::agent_card::TransportProtocol::JsonRpc => "json-rpc",
        }
    );
    ...
    println!(
        ...
        match card.authentication.scheme {
            motherbrain_a2a::agent_card::AuthScheme::None => "none",
        }
    );
    println!("  accepts:");
