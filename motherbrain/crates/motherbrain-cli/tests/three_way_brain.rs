...
//! # What this proves
//!
//! `dual_brain_pair.rs` proved 2 real `motherbrain a2a-serve` subprocesses
...
//! scoring pipeline — i.e., that `motherbrain score` with an A2A-sourced
...
//! 3. Run `motherbrain score` against the ecosystem registry as a
//!    subprocess.
...
fn spawn_peer(port: u16, project_root: &Path) -> ChildGuard {
    ...
    let child = Command::new(bin)
        ...
        .stderr(Stdio::piped())
        .spawn()
        .unwrap_or_else(|e| panic!("spawn motherbrain a2a-serve on {port}: {e}"));
...
#[tokio::test]
async fn ecosystem_motherbrain_score_aggregates_two_a2a_peers() {
    // Spawn two subprocess peers — each a full motherbrain a2a-serve process
    ...
    // Run `motherbrain score` against the ecosystem registry. This is the
    ...
    let output = Command::new(bin)
        ...
        .stdin(Stdio::null())
        .output()
        .expect("spawn motherbrain score");
    ...
    assert!(
        ...
        "motherbrain score exited non-zero. \
         stdout={stdout}\nstderr={stderr}"
    );
