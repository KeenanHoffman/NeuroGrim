...
| C10 frontend migration + legacy dispatcher deletion | 🔵 Multi-session per-variant | The "true" C10 final state: frontend handlers switch from listening on legacy IdeAction channel to broker:<...>:request events; `ipc/ide_action.rs` (1500+ lines) deletes. ~7-12 days per `IDE-LIFT-CLUSTER-MANIFEST.md` § "What C10 requires before completion". Per-variant testing in the live IDE is fundamentally operator-driven workflow. |
