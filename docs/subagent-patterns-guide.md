...
integration details, failure-handling policy, and the hook-system boundary.

Read the skill first. Come here when you need the examples, the calibration
blocks, or the failure semantics.

---

## When to Spawn vs. Run Inline (reprint with narrative)
...
**Legacy bare JSON** (existing patterns in this guide that predate the protocol):
...
- `.claude/skills/subagent-patterns.md` — the decision surface (the skill body that
  points here).
- `.claude/skills/pilot-protocol.md` — envelope schema (required before spawning).
- `.claude/skills/dual-review.md` — Pattern 3 worked example.
- `archived/skill-chain.md` — Chains 14, 15, 16 show parallel chain notation.
- `archived/hooks-reference.md` — hook system boundary documentation (historical).
- `archived/devops-philosophy.md` — Fail Fast / Shift Left and Defense in Depth (historical).
- `archived/lsp-subagent-queries.md` — original LSP-specific fan-out pattern that inspired
  Pattern 5 (historical; not load-bearing).
