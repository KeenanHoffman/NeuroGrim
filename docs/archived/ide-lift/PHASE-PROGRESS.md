...
| C9 per-leaf-op wire-up | ✅ Shipped (Tauri-emit pattern for 63; real impl for 1) | IDE commit `5a7f236`: each leaf-op routes via `emit_broker_request()` helper to a `broker:<broker>:<op>:request` event; frontend handler consumes + returns via response event. Federation/peer_ping has real in-broker implementation. Strangler-fig: legacy IdeAction dispatcher continues serving call sites until frontend migration consumes the broker event stream. |
| C10 substrate factory registration | ✅ Shipped | NeuroGrim commit `a1b21d4`: `BrokerHostConfig.broker_factories: BrokerFactoryRegistry` + `BrokerConfig.broker_type` field + `BrokerHost::boot` factory dispatch. IDE commit `ea1f7cc`: all 19 broker_types registered (V0 error-stub bodies pending S1-T AppHandle-capture extension). |
| C10 cluster.toml + per-broker manifests | ✅ Shipped doc | `NeuroGrim/docs/IDE-LIFT-CLUSTER-MANIFEST.md` — operator activation procedure with full cluster.toml + per-broker manifest template. Operator drops these into IDE project's `.claude/brain/broker/` to activate the registered brokers. |
| C10 frontend dual-dispatch | ✅ Shipped (14/14 call sites) | IDE commit `6383064`: every production `invoke("ide_action")` call site fires substrate dispatch alongside legacy. Strangler-fig migration window OPEN: substrate observes via trace.jsonl + governance + materializer; legacy stays authoritative. |
| C10 frontend handler migration + legacy deletion | 🔵 Operator-driven validation phase | Per-variant: operator validates substrate trace correctness; flips listener from legacy `invoke("ide_action")` reception to `listen("broker:<...>:request")` reception; deletes legacy line; after all 14 sites flip → deletes the legacy `ide_action` Tauri command + `ipc/ide_action.rs` (~1500 LOC). Estimated ~5-8 hours focused operator work per `IDE-LIFT-CALL-SITE-MIGRATION.md`. |
