...
**Rule (NeuroGrim's own surface):** every candidate Tier 3 function must be justified
as **unable to run as a pipeline**, not merely "simpler to write as a plain function."
The four functions listed above pass the test (the Pipeline Runner literally cannot
dispatch itself; the catalog loader has no catalog to consult). New Tier 3 additions
require a written justification in the proposing PR — operator review confirms there
is no Tier 2 path. Tier 3 size is the framework's audited attack surface; every line
added is reviewed for that reason.

**Consuming-project bootstrap autonomy (post-vision-audit clarification).** This
rule governs NeuroGrim's own Tier 3 surface; **consuming projects (cereGrim,
others) declare their own bootstrap-justification rubric per their cluster manifest
+ project review process.** A consuming project that needs its own bootstrap layer
(e.g., cereGrim's cognition-cycle initialization, dual-lobe consensus seeding) is
subject to its own operator review, not framework veto. The framework's rule is
"NeuroGrim Tier 3 must be small + justified; consuming projects' bootstrap is the
consuming project's decision under its own discipline." This preserves the
"methodology vs product" separation (NeuroGrim is the framework; consuming
projects compose on top with their own contracts).
...
**Governance composition is per-dispatch, not per-step (V0 implementation
clarification).** The 4 default governance pipelines below (`check-trust-budget`,
`check-kill-switch`, `record-dispatch`, `record-outcome`) run as **Runner-controlled
hooks before/after the entire pipeline dispatch**, not woven into the pipeline's step
sequence. This matches the spec's intent (Surfaced pipelines get full governance
composition) at lower implementation complexity. **Per-step governance** (e.g., a
kill-switch check BETWEEN expensive steps mid-pipeline) is a post-MVP extension via
a hypothetical `Step::Governance { check }` step type; not needed for V0. Sub-pipeline
dispatch re-runs the full per-dispatch governance loop, so nested sub-pipelines do
get governance per-call (not skipped).
...
|---|---|---|---|
| 1 | **Broker capsule** | `Broker` trait — async-trait + interior-mutability pattern (all methods take `&self`; brokers use `Arc<Mutex<W>>` inside for tick mutation). **Type-erased trait surface (V0 implementation decision):** `read_overlay()` returns `serde_json::Value`, NOT a typed associated type, so `Arc<dyn Broker>` is dyn-compatible + the Broker Registry holds heterogeneous brokers. Concrete brokers retain typed `Overlay<T>` + `WorkingState<W>` internally; JSON-serialize at the trait boundary. Loses Rust type-safety at consumer boundary; Materializer projects to Markdown anyway so the loss is minor where it matters. **Leaf-op dispatch (V0 pattern):** `execute_leaf(name, ctx)` per-broker method dispatches via `match name { "x" => ..., }` to leaf-op implementations. Canonical MVP pattern; brokers with ≥20 leaf-ops may benefit from a macro or separate dispatch-table builder (post-MVP ergonomic concern). | impl for one specific projection domain — typically ~30 lines glue + ~10-30 leaf-op functions |
...
| 6 | **Cold Store** | Trait over SQLite/JSONL backends (reuse). **Per-broker file isolation (R-O-4 closure, Phase 9):** the framework default is **per-broker cold-store file** (each broker has its own SQLite file or JSONL append-stream); SHARED cold-store files across brokers are explicitly forbidden — the SQLite-lock contention cascade under 10+ concurrent broker writes is the failure mode that motivated the isolation. **Backend guidance:** SQLite for single-broker (or low-tick-rate) deployments where transaction integrity matters more than write throughput; JSONL append-only for high-tick-rate deployments where lock contention dominates (trade-off: JSONL queries are slower; checkpoint reads require scanning). Cluster manifest declares per-broker backend; mixing within a cluster is permitted (Sensory Broker on JSONL for high-throughput, Work Broker on SQLite for transactional queries). **⚠ R-O-4 unvalidated in V0 implementation** (per V0-RETROSPECTIVE.md §C3): the V0 Work Broker uses in-memory `BacklogState`; no real SQLite or JSONL backend has been exercised. Validation pending — Wave 5.5 or S1-T must land a JSONL backend behind a feature flag + run the 10-broker contention benchmark per `broker-framework-backlog.md` BRK-04-COLD-STORE-CONTENTION test plan before the R-O-4 isolation claim is battle-stable. | Schema migration files (consumed by #26); cold-store backend choice per broker manifest |
...
| # | Building block | Framework provides | Broker author provides |
|---|---|---|---|
| 14 | **Broker Registry** | Discover/load brokers at startup; the manifest schema (incl. role-set declaration). **API surface (V0):** `load_manifests(cluster_path)` parses cluster + per-broker TOML; `register(broker: Arc<dyn Broker>)` adds a concrete broker instance (validates role-set against manifest); `validate()` post-registration check; `broker(id)` lookup; `iter_brokers()` iteration; **`full_catalog()` aggregates every registered broker's pipelines into one Vec for the Runner's per-dispatch consumption** (V0 implementation gap C5 closed). MVP doesn't dynamically load broker code; operator's main binary constructs concrete brokers + registers them per the methodology-vs-product separation. | A manifest declaration per broker (incl. its role-set); concrete broker instances passed via `register()` from the operator's main binary |
| 15 | **Tick Source** | Hook-driven + file-injection ticks; subscription API. **Operator-driven, NOT framework-fired (V0 implementation clarification):** the framework provides the tick PLUMBING (the `Broker::tick(WorldEvent)` trait method + subscription wiring) but does NOT autonomously generate ticks. Ticks are inherently external — something must HAPPEN to trigger them: a PostToolUse hook firing after Claude Code finishes a tool use; a file-watcher detecting a manifest change; an operator-issued `neurogrim broker tick` CLI command; or a scheduled timer if the operator sets one up. BB #15's job is to make the subscription model + dispatch path explicit; deciding WHEN to tick is operator-level configuration. | Subscribe to tick events the broker needs; operator-side tick triggers (hooks, watchers, schedulers) |
...
| 22a | **Materializer Composer** (named substrate, promoted from Tier 3) | Concatenates materializer segment files in **operator-declared order** into the auto-loaded `current-projection.md`. Order is operator-tunable per cluster manifest (`materializer_composition_order: [overlay, awareness-routing, ...]`). Each materializer owns one segregated file; the Composer enforces collision-safety by construction (no last-writer-wins). Promoted from implicit Tier 3 plain function because composition order matters for L1 context flow (per [`BROKER-AWARENESS.md`](BROKER-AWARENESS.md) §2 token budget). **Governance-first override (R-O-3 closure, Phase 9; transparency clarified post-vision-audit):** the Composer overrides operator's declared order to place `governance-pipelines` segment FIRST in the composed output, regardless of where the operator declared it. The reachability invariant (governance always reachable; per §4) is load-bearing for safety and outranks operator's composition preferences. **Operator agency preserved within that constraint:** operator's `materializer_composition_order` still controls the RELATIVE ordering of all non-governance segments; the framework's override is narrow (governance first; everything else as declared). **Transparency**: BB #32 Operator Telemetry projects a `composition-state` line showing "governance-segment-position: pinned-by-framework; non-governance order: <as-declared>" — operators see when the override is active and which segments are framework-controlled vs operator-controlled. Classification per [`CLUSTER-MANIFEST-SCHEMA.md`](CLUSTER-MANIFEST-SCHEMA.md) M-13 table: governance-first override is **Untunable** (code-change to alter); non-governance composition order is **OperatorOnly**. **Strict-mode enforcement (V0-RETROSPECTIVE.md §C4 closure):** the Composer accepts an `enforce_governance_segment_present: bool` cluster-manifest field (default `true` in production; `false` for first-boot / dev). When strict, missing the `governance-pipelines.md` segment file fails the compose loudly with `failure_reason: governance_segment_missing` rather than a soft warning. V0 ships with soft warning to ease onboarding; production deployments MUST flip to strict. **Auto-trigger after dispatch (V0-RETROSPECTIVE.md §C6 closure):** the Pipeline Runner exposes an `on_dispatch_complete` callback hook; operator's main binary registers a callback that re-runs the Materializer Composer after each successful dispatch (or after a configurable batch threshold for high-tick deployments). Decouples Runner ↔ Materializer: Runner doesn't depend on Composer; operator wires them per their cadence preference. **Truncation alarm:** if the composed total exceeds the configured context-window budget (per cluster manifest `materializer_context_budget`), the Composer emits a `materializer_truncation_imminent` event to BB #28 Diagnostics (audit_class: governance) AND falls back to **governance-only projection** — only the `governance-pipelines` segment is written to `current-projection.md`; capability segments are omitted with a one-line marker `[capability-segments-omitted-due-to-budget; see telemetry]`. Operator sees the alarm in BB #32 Telemetry; agent reasons under explicit safe-minimal awareness instead of partially-truncated awareness. Closes the silent governance-suppression path. | Composition order declaration + context-budget ceiling in cluster manifest |
...
**V0 implementation note — tunability declarations without Frame stack are
metadata-only (V0-RETROSPECTIVE.md §C2 closure).** Pipelines declare a tunability
tier (Untunable / OperatorOnly / OperatorConfirmed / Autonomous) in their catalog
entries; the framework enforces the basic semantics (Untunable cannot be set via
manifest per R-S-18 closure; OperatorConfirmed proposals route through the Proposal
Ledger per BB #21+#33). HOWEVER: the Frame-stack-driven governance behaviors the spec
also premises (e.g., "`stakes: production` auto-composes `require-operator-
confirmation`") REQUIRE BB #35 Frame stack to be implemented — and BB #35 is deferred
to post-MVP per the S\*-T plan. **Until BB #35 lands, tunability declarations are
declaratively-visible but Frame-driven behavior shifts do not yet activate.** Agents
+ operators see the tunability tier; the framework doesn't yet USE Frame context to
gate behavior beyond the basic tier semantics. Operator decision: ship V0 with
declarative-only tunability (current default) OR pull BB #35 into MVP scope (~3-5
days additional). The current default is "ship declarative; revisit when measurable
agent-experience friction emerges."
...
**Global escalation budget per turn (R-X-15 closure, Phase 9; softened post-vision-audit).** Recursion-guard
...
incident report for operator review. **Graceful-degradation behavior:** subsequent
escalations within the same turn are DENIED (the requesting broker receives the
refusal); the broker's pipeline either proceeds with its broker-default decision
(if one exists per its escalation contract) or rolls back via its `on_cancel`
handler. The TURN ITSELF does NOT auto-fail — work that doesn't depend on a
denied escalation continues. Workflow only transitions to `failed-escalation-cascade`
if a workflow step explicitly required an escalation that was denied AND has no
default fallback. This honors the principle that an escalation is a request, not a
mandatory blocker; the cluster recovers from cascade without total work loss.
Operator sees the cascade chain in telemetry + can either fix the pathological
...
The framework surfaces signals to operators across two tiers (B-59 backlog scopes
the implementation; this section pins the requirements). **Tiered to avoid the
cure-worse-than-the-disease failure mode** (post-vision-audit clarification):
adding 6 mandatory operator surfaces compounds the very burden it's trying to
reduce, so the framework requires only the minimum and treats the rest as
recommended-but-optional.

**Tier 1 — Minimum required (1 signal):**

The framework MUST surface ONE signal:

- **(1) Coherence-drift dashboard** (per below). Without this, operator has no
  early-warning signal for the burden risk this section addresses.

**Tier 2 — Recommended (5 signals; operator may enable selectively):**

The framework SHOULD surface these signals; operator enables per cluster manifest
(`operator_ux_surfaces.<surface_name>.enabled = true`). Defaults are disabled to
avoid burden-stacking; operator enables as deployment matures:
