...
- [x] Built-in Logs page: all 6 sources wired *(C-2 v2 + C-3 expansion: invocation-ledger + notifications + score-history + services.jsonl. Services.jsonl persistence layer added in `crate::services` — start/stop/readiness handlers append on each terminal-state transition; the file is also durable across dashboard restarts, laying groundwork for v5 service-restart recovery.)*
...
### S15-C-3: Built-in Logs page (3 days) — ✅ ALL 6 SOURCES SHIPPED (v1: publish-gates + approvals; v2 expansion: invocation-ledger + notifications + score-history + services.jsonl). Remaining polish: toast notifications + per-row drill-down sidebar.
