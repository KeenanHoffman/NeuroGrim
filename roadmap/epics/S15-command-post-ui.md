...
- [x] Built-in Logs page: all 6 sources + per-row drill-down + cross-page toasts *(C-2 v2 + C-3 expansion: invocation-ledger + notifications + score-history + services.jsonl, plus a click-row-to-open modal that surfaces the full original record per source — publish-gate exit codes + error_detail, approval decision metadata, invocation session/invocation ids, notification full payload, score-history snapshot detail, services lifecycle reason — with raw JSON fallback. Services.jsonl persistence layer added in `crate::services` — start/stop/readiness handlers append on each terminal-state transition; durable across dashboard restarts, laying groundwork for v5 service-restart recovery. Toast notifications dispatch on `service_failed` SSE events so operators on other pages don't miss peer crashes; the ToastProvider + dispatchToastForEvent infrastructure is reusable for future toast triggers when adopters identify other "operator-would-miss-this" events.)*
...
### S15-C-3: Built-in Logs page (3 days) — ✅ FEATURE-COMPLETE (v1: publish-gates + approvals; v2 expansion: invocation-ledger + notifications + score-history + services.jsonl + per-row drill-down modal + cross-page toast notifications via SSE). All 6 sources wired, drill-down shipped, ambient awareness via toasts shipped.
