...
### S6-DB-1: motherbrain-a2a Crate Scaffold

**Status:** **Complete** (2026-04-17)
**Effort:** L
**Depends on:** S5-TP-8 (schemas must exist)

Created the `motherbrain-a2a` Rust crate with the A2A protocol primitives: envelope,
...
- `cargo check -p motherbrain-a2a` — clean
- `cargo test -p motherbrain-a2a` — **19/19 tests pass**:
...
**Acceptance criteria:**
- [x] `cargo test -p motherbrain-a2a` passes — 19/19 green
...
coordinated pieces** to honor the "zero I/O" invariant on `motherbrain-core`:

1. **`motherbrain-core/src/ecosystem.rs`** (~700 lines) — pure types + algorithms:
...
2. **`motherbrain-ecosystem`** — NEW async I/O crate:
   ...
   - A2A branch: `motherbrain_a2a::TaskClient` + `snapshot.requested` → `score.updated`
...
**Architectural decision:** Keep `motherbrain-core` strictly pure (zero I/O). All dispatch
I/O lives in the new `motherbrain-ecosystem` crate. Deserialization into the Rust
...
- `cargo test -p motherbrain-core ecosystem::tests` — 21/21 pass
- `cargo test -p motherbrain-ecosystem` — 2 lib + 4 contract tests = **6/6 pass**
...
- **Boundary check:** zero `rmcp` / `motherbrain_mcp` imports in `motherbrain-ecosystem` or `motherbrain-core::ecosystem`

**Collateral fix:** three pre-existing `motherbrain-core::registry::tests` tests referenced
...
Shipped three new CLI subcommands under `motherbrain-cli` plus closed the Phase C
Agent Card discovery FIXME in `motherbrain-ecosystem::invoke_a2a`.

**Files:**
- `crates/motherbrain-cli/src/commands/a2a_serve.rs` (~260 lines) — Agent Card builder from `BrainRegistry` + `TaskServer` wiring + handlers
- `crates/motherbrain-cli/src/commands/a2a_invoke.rs` (~195 lines) — `discover → invoke` one-shot client
- `crates/motherbrain-cli/src/commands/a2a_discover.rs` (~85 lines) — pretty-print Agent Card
- `crates/motherbrain-cli/tests/a2a_cli.rs` (~105 lines) — end-to-end integration test
- `crates/motherbrain-a2a/src/client.rs` — added `discover_at(endpoint, override_url)` alongside existing `discover` (honors registry `agent_card_url` override per Appendix G.3 step 1)
- `crates/motherbrain-ecosystem/src/lib.rs` — closed FIXME: `invoke_a2a` now discovers Agent Card + validates `capabilities.accepts` + validates `interface_version` before POST; new error variants `CapabilityMismatch`, `InterfaceVersionMismatch`
- `crates/motherbrain-ecosystem/tests/contract.rs` — two new discovery-validation tests
- `crates/motherbrain-cli/{Cargo.toml, src/main.rs, src/commands/mod.rs}` — wiring
...
- [x] Idempotency: `idempotent_post_returns_cached_without_rehandler` in `motherbrain-a2a` still passes
- [x] **Bonus:** `motherbrain a2a-invoke` and `motherbrain a2a-discover` CLI subcommands exist
...
- **Live end-to-end verified against the Meta Brain registry** —
  `motherbrain a2a-serve` + `motherbrain a2a-invoke` produces a real scorecard
...
Shipped: end-to-end fractal composition proof — two actual `motherbrain a2a-serve`
...
**File:** `crates/motherbrain-cli/tests/dual_brain_pair.rs` (~493 lines including helpers)

**Tests added (3):**
1. **`fractal_composition_end_to_end_over_loopback`** — spawns two `motherbrain a2a-serve`
...
**Verification (2026-04-17):**
- `cargo test -p motherbrain-cli --test dual_brain_pair` — **3/3 pass in 2.60s**, three consecutive runs
- `cargo test --workspace` — **176 pass, 0 fail** (baseline 173, +3 new)
- Boundary grep (`rmcp|motherbrain_mcp`) on `dual_brain_pair.rs` — clean
...
- [x] Two actual `motherbrain` processes (spawned via `env!("CARGO_BIN_EXE_motherbrain")`)
...
- `Dockerfile` (172 lines) — multi-stage: `rust:1.89-slim-bookworm` builder →
  ...
  `EXPOSE 8421`. `ENTRYPOINT ["motherbrain"]` + default `CMD ["a2a-serve", "--port", "8421", "--bind", "0.0.0.0", "--project-root", "/brain"]`.
...
- `docker-compose.yml` (81 lines) — dual-brain pair: `motherbrain-local` on host
  port 8421, `motherbrain-external` on host port 8422. Same image, different mount
...
- `motherbrain-local-project/.claude/` + `motherbrain-external-project/.claude/` —
...
- `motherbrain a2a-serve --bind <addr>` — default `127.0.0.1` (preserves every
...
- Switched `reqwest` in `motherbrain-a2a`, `motherbrain-ecosystem`, `motherbrain-cli`
...
**Verification (2026-04-17):**
- `docker build -t motherbrain:dev .` — **145 MB image** (target <200 MB: PASS)
- First cold build: 4m 6s; incremental rebuild: ~2s
- `docker run motherbrain:dev` + `curl /.well-known/agent-card.json` — real Agent Card served
- `docker compose up` — both containers running, `motherbrain a2a-invoke` against
...
- Boundary check: deployment artifacts don't create a new MCP dependency path;
  ...
  (motherbrain-mcp is part of the workspace and must be buildable)

**Acceptance criteria:**
- [x] Reference deployment doc exists
- [x] Dockerfile builds a working `motherbrain a2a-serve` container
- [x] Worked example: dual-brain pair via `docker compose up` + host `motherbrain a2a-invoke` proves round-trip
...
1. **Stub-binary-shipped bug:** the `cargo build --release` deps-caching trick
   ...
   the Dockerfile's sanity-check (`motherbrain --version`) being empty. Fixed
...
`D:\Brains\` becomes an **ecosystem Brain** that coordinates MotherBrain (code) and
...
- Overwrote stale MotherBrain-copy content: `brain-registry.json` + 5 stale CMDBs

**Six ecosystem-only domains:**

| Domain | What it sees |
|--------|--------------|
| `spec-impl-alignment`   | MotherBrain conforms to LSP-Brains spec at current version |
...
- [x] Stale MotherBrain-copy CMDBs removed; preserved files (settings, human-comms, secret-refs) untouched
