...
| **11. Supply-chain security (MASTER GATE)** | 🔴 **E-SC-0/1/2/3/4/5/6/7/8 GREEN; E-SC-9/10 pending.** No `cargo publish` until this closes. 11-epic scaffolding at `~/.claude/plans/parallel-hugging-eich.md`. **Layers 1+2+3 + spec all shipped + calibration framework shipped.** Phase 0 self-audit baseline + Rust SDK reframe + Layer-1 SCA covering Rust+Python+Node + Layer-2 vigilance (7 sub-sensors) + Layer-3 review framework + LSP-Brains v2.6 §16 + METH-EV §15. **Calibration framework shipped (E-SC-8)**: fixture library (5 L1 + 8 L2 + 4 L3 fixtures) + per-layer harness + `neurogrim sca-calibrate` CLI + JSON report + `--check-promotion-ready` gate (returns exit 1 in v1 by design — honest signal that calibration evidence is below ≥30-fixture threshold + L3 has 0 days of triage data). v1 dogfood baseline: pass-with-sample-size-warning across all three layers; promotion-not-ready (5 gaps documented). Remaining: E-SC-9 (container docs), E-SC-10 (final dogfooding + publish gate). |
