...
| **11. Supply-chain security (MASTER GATE)** | 🔴 **E-SC-0/1/2/3/4/5/7 GREEN; E-SC-6/8/9/10 pending.** No `cargo publish` until this closes. 11-epic scaffolding at `~/.claude/plans/parallel-hugging-eich.md`. Phase 0 self-audit baseline + Rust SDK reframe + native Layer-1 SCA sensor covering all three ecosystems (Rust crates.io + Python PyPI + Node npm/yarn/pnpm) with OSV + pinned RustSec advisory-db, accepted-advisories triage, count-based scoring, MCP wrapper, multi-ecosystem dispatch. **Layer 2 vigilance shipped** (E-SC-5 commit `0e79680`): all 7 sub-sensors (typosquat / publish-cadence / maintainer-delta / transitive-surface / signature-gap / binary-reproducibility / exfil-indicator) with 7-day registry cache + opt-in tarball-fetch sensors + dogfood-tuned FP suppression. **Spec normative work also shipped** (LSP-Brains v2.6 §16 Supply-chain awareness + METH-EV §15 + 2 new schemas + A2A enum extensions; LSP-Brains commit `5ceab43`). Remaining: E-SC-6 (Layer 3 agent-assisted review), E-SC-8 (calibration), E-SC-9 (container docs), E-SC-10 (final dogfooding + publish gate). |
