...
//!
//! Renders the loaded `BrainContext` (registry + scored AgentOutput +
//! project_root) as a compact prose summary tuned for AI agents entering
//! an unfamiliar Brain. Same upstream data as the JSON `agent` output;
...
//!
//! Color handling: the `plain: bool` flag disables ANSI colors entirely;
//! when false, the `colored` crate's TTY auto-detection (NO_COLOR /
//! piped-stdout heuristics) handles non-terminal output gracefully.
...
/// Render the entire prose orientation for an agent. `plain=true`
/// suppresses all ANSI color escapes — required when stdout is piped
/// into another tool's context.
pub fn display_prose(ctx: &BrainContext, plain: bool) {
    if plain {
        // Force-disable color globally for this process — covers any
        // `colored` call paths that don't take an explicit plain flag.
        colored::control::set_override(false);
    }

    section_identity(ctx, plain);
    section_current_state(ctx, plain);
    section_strongest_signals(ctx, plain);
    section_calls_to_action(ctx, plain);
    section_skills(ctx);
    section_hats(ctx, plain);
    section_federation(ctx, plain);
    section_footer(plain);
}

// --- Section 1: Brain identity -----------------------------------------

fn section_identity(ctx: &BrainContext, plain: bool) {
    let project_label = if !ctx.registry.meta.description.is_empty() {
        // Use the first sentence of the description (or up to ~80 chars).
        first_sentence(&ctx.registry.meta.description, 80)
    } else {
        ctx.project_root
            .file_name()
            .and_then(|s| s.to_str())
    ...
    let total = ctx.registry.config.domain_weights.len();
    let weighted = ctx
        .registry
        .config
        .domain_weights
    ...
    let path = format!(" ({})", ctx.project_root.display());

    if plain {
        println!("{}{}", header, path);
    } else {
        println!("{}{}", header.bold(), path.dimmed());
    }
...
fn section_current_state(ctx: &BrainContext, plain: bool) {
    let out = &ctx.agent_output;
    let weighted = ctx
        .registry
        .config
        .domain_weights
        ...
        .filter(|w| **w > 0.0)
        .count();
    let is_all_advisory = weighted == 0 && !ctx.registry.config.domain_weights.is_empty();
    ...
    if is_all_advisory {
        ...
        let label = ScoreLabel::from_score(out.score, 75, 50);
        let score_str = format!("{}/100", out.score);
        ...
        let conf_note = if out.unified_confidence < 100 {
            format!(" (confidence: {}%)", out.unified_confidence)
        } else {
            String::new()
        };
        println!("  Score: {}{}", colored_score, conf_note);
    }

    let traj_note = match &out.trajectory {
        Some(t) => format!(
...
fn section_strongest_signals(ctx: &BrainContext, plain: bool) {
    let mut entries: Vec<(&String, u8, u8, f64)> = ctx
        .agent_output
        .domains
        .iter()
        .map(|(k, d)| (k, d.effective_score, d.confidence, d.weight))
        .collect();
    // Stable sort: effective_score desc, then weight desc, then name asc.
    entries.sort_by(|a, b| {
        b.1.cmp(&a.1)
    ...
    if top.is_empty() {
        println!("Strongest signals: (no domains scored)");
        println!();
        return;
    }
...
fn section_calls_to_action(ctx: &BrainContext, plain: bool) {
    let recs = &ctx.agent_output.top_recommendations;
    if recs.is_empty() {
        println!("Calls to action: (none queued)");
        println!();
        return;
    }
    println!("Calls to action:");
...
fn section_skills(ctx: &BrainContext) {
    let skills_dir = ctx.project_root.join(".claude").join("skills");
    let entries = scan_skills(&skills_dir);
    if entries.is_empty() {
        println!("Available skills: (none discovered in .claude/skills/)");
        println!();
        return;
    }
    println!("Available skills (in .claude/skills/):");
...
/// heading. Handles plain scalars (`description: text`) and folded
/// scalars (`description: >-` followed by indented continuation lines).
/// Truncates to 100 chars to keep the prose layout tight.
    ...
    if let Some(rest) = content.strip_prefix("---\n") {
        if let Some(end) = rest.find("\n---") {
            ...
            for (i, line) in lines.iter().enumerate() {
                ...
                let val = val.trim();
                // YAML folded scalar (`>-`) or literal block (`|-`) —
                // gather subsequent indented continuation lines.
...
fn section_hats(ctx: &BrainContext, plain: bool) {
    let hats = &ctx.registry.config.hats;
    if hats.is_empty() {
        println!(
...
fn section_federation(ctx: &BrainContext, plain: bool) {
    let children = ctx
        .registry
        .config
        .extra
    ...
    let Some(children) = children else {
        println!("Federation peers: none declared");
        println!();
        return;
    };
    ...
    println!("Federation peers ({}):", children.len());
    ...
    for (id, val) in entries {
        ...
        println!("  {} {} @ {}{}", arrow, id_part, endpoint_part, suffix);
