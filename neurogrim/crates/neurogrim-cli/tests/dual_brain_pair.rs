...
//! two real `neurogrim a2a-serve` processes can talk A2A over loopback and
...
use neurogrim_core::ecosystem::{ChildEntry, ChildStatus, ChildTransport, EcosystemRegistry};
use neurogrim_ecosystem::score_ecosystem;
use serde_json::json;
use url::Url;
...
/// `neurogrim.exe` processes don't accumulate across test runs.
struct ChildGuard {
    name: String,
    child: Option<Child>,
}
...
/// Spawn `neurogrim a2a-serve --port N --project-root <dir>` as a real
/// subprocess. Returns a `ChildGuard` that kills the process on drop.
///
/// We locate the binary via `env!("CARGO_BIN_EXE_neurogrim")` — cargo
...
fn spawn_peer_server(port: u16, project_root: &Path) -> ChildGuard {
    ...
    let child = Command::new(bin)
        ...
        .stderr(Stdio::piped())
        .spawn()
        .unwrap_or_else(|e| panic!("spawn neurogrim a2a-serve on port {port}: {e}"));
...
/// **Headline test.** Two real `neurogrim a2a-serve` processes, each
...
async fn dual_brain_envelope_validates_against_schema_at_every_hop() {
    ...
    // `neurogrim_ecosystem::invoke_child` which returns the raw
    ...
    let child_output = neurogrim_ecosystem::invoke_child(&entry)
        .await
