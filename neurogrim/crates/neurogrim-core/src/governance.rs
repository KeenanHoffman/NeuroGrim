...
fn derive_command(domain_key: &str, registry: &BrainConfig) -> String {
    ...
    if BUILTIN_SENSORY_TOOLS.contains(&domain_key) {
        return format!("neurogrim sensory {}", domain_key);
    }
    ...
    // 4. Generic fallback
    format!(
        "# Improve {} — see neurogrim health for details",
        domain_key
    )
}
...
mod tests {
    ...
    fn command_derived_for_builtin_tools() {
        ...
        assert_eq!(recs.len(), 1);
        assert!(
            recs[0].command.contains("neurogrim sensory test-health"),
            "command should reference neurogrim sensory for built-in tools"
        );
    }
