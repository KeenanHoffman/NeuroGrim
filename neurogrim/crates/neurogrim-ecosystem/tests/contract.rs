...
use neurogrim_ecosystem::{invoke_child, score_ecosystem};
...
/// `neurogrim_mcp`. Doing it as a test means it runs in CI without any
...
fn boundary_no_mcp_imports_in_lib_source() {
    ...
    for needle in ["rmcp", "neurogrim_mcp"] {
        assert!(
            !src.contains(needle),
            "neurogrim-ecosystem/src/lib.rs must not reference {needle:?}; found a match"
        );
    }
}
