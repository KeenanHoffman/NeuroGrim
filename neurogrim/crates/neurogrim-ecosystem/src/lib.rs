...
//! (spec §9 + §13.5). `neurogrim-core::ecosystem` holds the pure types and
...
pub enum EcosystemError {
    ...
    /// `neurogrim-core::ecosystem::topological_sort`.
...
///   call the peer via `neurogrim-a2a::TaskClient`. The reply envelope's
...
/// with [`neurogrim_core::ecosystem::enabled_only`] or by inspecting
...
async fn invoke_a2a(
    ...
    // in neurogrim-a2a test with snapshot.delivered as the reply_to form.
...
pub use neurogrim_core::ecosystem::merge_cross_project_variables;

// Re-export the status enum so callers don't need two imports.
pub use neurogrim_core::ecosystem::ChildStatus;
